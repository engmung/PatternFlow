...
namespace Origin {
  ...
  struct Params {
    float hue      = 0.0f;
    float speed    = 2.0f;
    float scale    = 1.0f;   // was tiles/spread — now pixel scale
    float displace = 0.0f;   // pixel displacement offset
  };
  Params params;
  ...
  void update(float dt, const InputFrame& input) {
    ...
    // Knob3: scale (0.5..4.0) — zooms the sprite pixels
    int d2 = input.knobDeltas[2];
    if (d2 != 0)
      params.scale = constrain(params.scale + d2 * 0.1f, 0.5f, 4.0f);
    if (input.btnPressed[2]) params.scale = 1.0f;

    // Knob4: displace (±32px) — shifts sprite origin
    int d3 = input.knobDeltas[3];
    if (d3 != 0)
      params.displace = constrain(params.displace + d3 * 1.0f, -32.0f, 32.0f);
  ...
  void draw() {
    // Pixel size on panel driven by scale knob
    float cellW = params.scale;
    float cellH = params.scale;

    // Center the sprite, then apply displacement offset
    float originX = (PANEL_RES_W - BMP_W * cellW) * 0.5f + params.displace;
    float originY = (PANEL_RES_H - BMP_H * cellH) * 0.5f;
    ...
    for (int y = 0; y < BMP_H; y++) {
      for (int x = 0; x < BMP_W; x++) {
        uint8_t idx = LED[y][x];
        if (idx != 1) continue;   // ← fixed: skip everything that isn't an "on" pixel

        float br = PALETTE[0][0];
        float bg_= PALETTE[0][1];
        float bb = PALETTE[0][2];

        float wave  = fastSin(phase * 2.0f + x / 3.0f + y / 3.0f);
        float pulse = 0.5f + 0.5f * wave;

        uint8_t fr, fg, fb;
        shiftColor(br * pulse, bg_ * pulse, bb * pulse, params.hue, fr, fg, fb);

        int px0 = (int)(originX + x * cellW);
        int py0 = (int)(originY + y * cellH);
        int px1 = (int)(originX + (x + 1) * cellW);
        int py1 = (int)(originY + (y + 1) * cellH);
